...
class LowRankAdapter(nn.Module):
    """
    Low-rank adapter: Δ(h) = α * W_up( act( W_down(h) ) )
...
class AdapterSteerer(nn.Module):
    ...
    def __init__(
        ...
        alpha: float = 1.0,
        activation: str = "silu",
        inference_mode: str = "all",  # "all", "lora_only", "adapter_only"
    ):
        super().__init__()
        assert hasattr(base_model, "model"), "Assuming LLaMA-style with .model.layers"
        assert apply_to in ("last", "all")
        assert inference_mode in ("all", "lora_only", "adapter_only")
        ...
        for layer_idx in self.layers_to_steer:
            try:
                ...
                if hasattr(self.base_model.model, 'language_model'):
                    layer = self.base_model.model.text_model.layers[layer_idx]
                else:
    ...
    def _set_lora_enabled(self, enabled: bool):
        if enabled:
            if hasattr(self.base_model, "enable_adapter_layers"):
                self.base_model.enable_adapter_layers()
                return
            if hasattr(self.base_model, "enable_adapter"):
                self.base_model.enable_adapter()
                return
            if hasattr(self.base_model, "set_adapter"):
                adapter = getattr(self.base_model, "active_adapter", None) or "default"
                self.base_model.set_adapter(adapter)
                return
        else:
            if hasattr(self.base_model, "disable_adapter_layers"):
    ...
    def set_inference_mode(self, mode: str):
        assert mode in ("all", "lora_only", "adapter_only")
